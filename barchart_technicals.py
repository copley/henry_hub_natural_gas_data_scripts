...
def approximate_price_for_stoch(df, period, target_stoch=80, k_smooth=3, d_smooth=3):
    ...
    for _ in range(30):
        ...
        raw_stoch, _, _ = compute_stochastics(
            test_df, period=period, k_smooth=k_smooth, d_smooth=d_smooth
        )
...
def build_expanded_cheatsheet(df):
    ...
    (
        ...
        pivot_s2,
        pivot_s3,
    ) = compute_pivots_s_r(the_high, the_low, last_close)
...
def decide_trade_action(latest_price):
    ...
    A user changes these levels each week.
    - Key Support: 3.965
...
class IBApp(EWrapper, EClient):
    ...
    def print_historical_data_info(self):
        ...
        print(
            "The script’s results are best viewed as a snapshot of the market’s technical state for the next 24 hours"
        )
        print("==============================================")
    ...
    def print_last_bar_timestamp(self, df):
        ...
        if dt_field is not None and isinstance(dt_field, datetime):
            ...
            print(
                f"The last bar in the data set is from {dt_field.strftime('%Y-%m-%d %H:%M:%S')}"
            )
        else:
    ...
    def print_analysis_validity(self, validity_days=1):
        ...
        print("==============================================")
        print(
            f"Analysis Timestamp     : {analysis_timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def process_data(self):
        ...
        try:
            ...
            print(f"\n[DEBUG] Received {len(df)} bars from IB.")
            if df.empty:
            ...
            print("=" * 46)
            print(" Historical Data Set Information")
            print(" (Note: Best viewed as a snapshot for ~24 hours) ")
            print("=" * 46)
            if not self.historical_data:
                print("No historical data available.")
            else:
                # Get first/last bar info
                first_bar = self.historical_data[0]
                last_bar = self.historical_data[-1]
                first_dt = first_bar["datetime_obj"]
                last_dt = last_bar["datetime_obj"]
                ...
                bar_size = "1 Day"  # your current bar size
                duration = "1 Year"  # your current duration
                symbol = self.create_mes_contract().symbol
            ...
            print(f"  Prediction Validity    : {validity_days} day(s)")
            print(
                f"  Prediction Valid Until : {valid_until.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            print("=" * 46, "\n")
            ...
            if not df.empty:
                ...
                dt_field = last_bar.get("datetime_obj", None)
                print("=" * 46)
                print(" Last Bar Timestamp")
                print("=" * 46)
                if dt_field is not None and isinstance(dt_field, datetime):
                    print(
                        f"  Most recent bar in dataset: {dt_field.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                else:
                    raw_date = last_bar["date"]
                    print(f"  The last bar in the data set has a date: {raw_date}")
            ...
            print("=" * 60, "\n")
            print("PERIOD |   Moving Avg |  Price Change |  % Change  | AvgVolume")
            print("-------+--------------+---------------+------------+----------")
            ...
            for (label, ma_col, pc_col, pct_col, vol_col) in [
                ...
                ma_val = safe_round(last_row[ma_col], 2)
                pc_val = safe_round(last_row[pc_col], 2)
                pct_val = safe_round(last_row[pct_col], 2)
                vol_val = int(safe_round(last_row[vol_col], 0) or 0)

                print(
                    f"{label:<7}|{ma_val:>12} |{pc_val:>14} |{pct_val:>7}%   |{vol_val:>7}"
                )

            # ------------------------------
            # Additional blocks: STOCH / RSI / etc.
            # ------------------------------
            print("\nPERIOD  | Raw Stoch  |   %K    |   %D    |  ATR(14)")
            print("--------+------------+---------+---------+---------")
            for (label, rcol, kcol, dcol, atr_col) in [
                ...
                r_val = safe_round(last_row[rcol], 2)
                k_val = safe_round(last_row[kcol], 2)
                d_val = safe_round(last_row[dcol], 2)
                atrval = safe_round(last_row[atr_col], 2)
                print(f"{label:<7}|{r_val:>10}% |{k_val:>7}% |{d_val:>7}% |{atrval:>8}")

            print("\nPERIOD  | RSI      |  %R      | HV(20)   | MACD (hist)")
            print("--------+----------+----------+----------+-----------")
            for (label, rsi_col, pr_col, hv_col) in [
                ...
                rsi_val = safe_round(last_row[rsi_col], 2)
                pr_val = safe_round(last_row[pr_col], 2)
                hv_val = safe_round(last_row[hv_col], 2)
                macd_val = safe_round(last_row["macd"], 2)
                print(
                    f"{label:<7}|{rsi_val:>8}% |{pr_val:>8}% |{hv_val:>8}% |{macd_val:>8}"
                )
            ...
            (
                ...
                pivot_s2,
                pivot_s3,
            ) = compute_pivots_s_r(last_row["high"], last_row["low"], last_row["close"])
            stdev_dict, avg_5day = compute_std_devs(df["close"], [1, 2, 3])
            p1_res = avg_5day + stdev_dict[1]
            p1_sup = avg_5day - stdev_dict[1]
            p2_res = avg_5day + stdev_dict[2]
            ...
            print(
                f"   R1: {safe_round(pivot_r1,2)}   R2: {safe_round(pivot_r2,2)}   R3: {safe_round(pivot_r3,2)}"
            )
            print(
                f"   S1: {safe_round(pivot_s1,2)}   S2: {safe_round(pivot_s2,2)}   S3: {safe_round(pivot_s3,2)}"
            )
            print("\n 5-Day Avg Price (for SD calc):", safe_round(avg_5day, 2))
            print(
                f" Price 1 SD Resistance: {safe_round(p1_res,2)}   Support: {safe_round(p1_sup,2)}"
            )
            print(
                f" Price 2 SD Resistance: {safe_round(p2_res,2)}   Support: {safe_round(p2_sup,2)}"
            )
            print(
                f" Price 3 SD Resistance: {safe_round(p3_res,2)}   Support: {safe_round(p3_sup,2)}"
            )
            print("==================================================\n")
            ...
            # Store final for possible usage
            self.final_technical_data = {
                "last_close": last_row["close"],
                "ma_20": safe_round(last_row["MA_20"], 2),
                "ma_50": safe_round(last_row["MA_50"], 2),
                "rsi_14": safe_round(last_row["rsi_14"], 2),
                "macd": safe_round(last_row["macd"], 2),
                "atr_14": safe_round(last_row["atr_14"], 2),
            ...
            # ==========================
            # 7) EXTENDED CHEAT SHEET
            # ==========================
            cheat_sheet_rows = build_expanded_cheatsheet(df)
            print("======== EXTENDED TRADER'S CHEAT SHEET ========")
            ...
            atr_val = self.final_technical_data["atr_14"] or 0.15
            ...
            print("=" * 60)
            print("         TRADE DECISION (Rogue Radar Logic)")
            print("=" * 60)
            ...
            def interpret_rsi(rsi_value):
                ...
                if rsi_value < 30:
                    return "RSI < 30 => Oversold condition. Possible rebound."
                elif rsi_value > 70:
                    return "RSI > 70 => Overbought condition. Pullback risk."
                else:
                    return "RSI 30-70 => Neutral range."

            def interpret_macd(macd_val):
                if macd_val is None:
                    return "No MACD data available."
                if macd_val > 0:
                    return "MACD > 0 => Bullish momentum."
                else:
                    return "MACD < 0 => Bearish momentum."

            def interpret_atr(atr_val):
                if atr_val is None:
                    return "No ATR data available."
                if atr_val > 0.5:
                    return "ATR > 0.5 => Elevated volatility."
                else:
                    return "ATR <= 0.5 => Mild volatility."

            print("============ INTERPRETATIONS / ANALYSIS SUMMARY ============")
            rsi_comment = interpret_rsi(self.final_technical_data["rsi_14"])
            macd_comment = interpret_macd(self.final_technical_data["macd"])
            atr_comment = interpret_atr(self.final_technical_data["atr_14"])
            print(
                f" [RSI (14d)] : {self.final_technical_data['rsi_14']} => {rsi_comment}"
            )
            print(
                f" [MACD Hist] : {self.final_technical_data['macd']} => {macd_comment}"
            )
            print(
                f" [ATR (14d)] : {self.final_technical_data['atr_14']} => {atr_comment}"
            )
            print(
                f" [Barchart]  : {opinion_results['overall_opinion']}"
                f" (Short={opinion_results['short_avg']}, "
                f"Med={opinion_results['medium_avg']}, "
                f"Long={opinion_results['long_avg']})"
            )
            print(f" [RogueRadar]: {trade_decision} => (Thresholds: 3.965 / 4.174)")
            print("============================================================\n")
