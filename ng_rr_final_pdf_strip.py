...
def main():
    ...
    # Step 1: Extract table data from the first table page (page index 0)
    table_data, page_text = extract_table_text(pdf_path)

    # Step 2: Extract text from non-table pages (pages 1, 2, 4, and 5)
    ...
    # Step 3: Render PDF page 0 to PNG (for circle detection)
    out_png = "page.png"
    ...
    # Step 5: Construct a new output filename based on the input PDF name + time+date
    pdf_input_name = os.path.splitext(os.path.basename(pdf_path))[0]  # e.g. "Rogue_Radar_20250327"
    now_str_time = datetime.now().strftime("%H%M%S")  # e.g. "215800"
    now_str_date = datetime.now().strftime("%d_%B")   # e.g. "01_April"
    
    # Final filename e.g. Rogue_Radar_20250327_215800_01_April.json
    output_filename = f"{pdf_input_name}_{now_str_time}_{now_str_date}.json"

    # Step 6: Save final JSON with the new naming scheme
