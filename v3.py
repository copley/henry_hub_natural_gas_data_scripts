...
HDD_BASE_TEMP = 65
FORECAST_DAYS = 7
HISTORICAL_YEARS = 1  # how many years of data to fetch from NOAA
...
def fetch_noaa_historical_daily_data(regions_dict, years=1, noaa_token=NOAA_API_TOKEN):
    """
    [NEW FUNCTION]
    Fetch daily NOAA data (TMIN/TMAX) for each city in each region for ~1 year,
    compute daily HDD, and return a single DataFrame with columns:
        [date, region, HDD].

    This function is for correlation analysis, so we keep daily data
    ...
    # 1) group by [date, region], compute Weighted HDD across cities
    #    Weighted HDD = sum(city_HDD * city_population) / sum(region_population)
    df_all = df_all.merge(
        pd.DataFrame([
...
def fetch_ng_futures_data(start_date, end_date):
    ...
    app.reqHistoricalData(
        ...
        durationStr="180 D",   # <--- ADJUST AS DESIRED
    ...
    # If IB returns strings like '20230401', parse them accordingly.
    # Often IB returns strings like '2023-04-01', so let's handle generically:
...
def main():
    ...
    # 2) Also fetch daily NOAA data for correlation (new function)
    ...
    # ----------------
    # We'll do the same step as before: get short-term forecasts
    CITY_COORDS = {
    ...
    # 5) Calculate total Gas Weighted Degree Days across all regions for the 7-day forecast
    gwdd_df = calculate_gwdd(region_forecast_dfs, region_noaa_avgs)

    # ----------------
    # PART E: Correlation Analysis
    # ----------------
    # If we have daily HDD data and daily price data, let's do correlation.
    if not df_region_daily_hdd.empty and not ng_prices_df.empty:
        ...
        # 2) Merge with NG daily closes
        df_merged = pd.merge(df_hdd_pivot, ng_prices_df[["Date", "Close"]].rename(columns={"Date":"date"}),
                             on="date", how="inner")

        df_merged.sort_values("date", inplace=True)
        # 3) Compute daily % change for Price and each region's HDD
        ...
        cols_of_interest = ["Price_Change"] + [f"{r}_Change" for r in REGIONS if r in df_merged.columns]
        corr_matrix = df_merged[cols_of_interest].corr()
        logging.info("\nCorrelation Matrix (Daily % Changes):")
        logging.info(f"\n{corr_matrix}")

        # 5) Scenario matrix example
        #   We'll treat each region's HDD_Change separately:
        scenario_df = scenario_matrix(
            df_merged,
            [f"{r}_Change" for r in REGIONS if f"{r}_Change" in df_merged.columns],
            hdd_changes=[-20, -10, 0, 10, 20]  # e.g. -20%, -10%, 0, +10%, +20%
        )
