...
def safe_request(
    ...
    :return: Response object if successful, None if failed after all retries
    """
...
def fetch_noaa_historical_data(regions_dict, years=1, noaa_token=NOAA_API_TOKEN):
    ...
    city_avg_hdds = {}
    max_failures = 3  # how many city-level total failures we allow before we skip
    fail_count = 0
        ...
        for city in cities:
            ...
            while current_start < end_date:
                ...
                while True:
                    ...
                    # Make the request with safe_request
                    ...
                    if r is None:
                        ...
                        break  # exit pagination loop, move to next chunk
            ...
            if frames:
                ...
                if fail_count >= max_failures:
                    ...
                    return {}  # or break out/exit if you prefer
                continue
...
class IBApp(EWrapper, EClient):
    ...
    def error(self, reqId, errorCode, errorString):
        """
        Basic IB error handler override.
        You can add logic here to handle certain error codes more gracefully.
        """
...
def main():
    ...
    logging.info("Retrieving 1-year average HDD from NOAA for each region (weighted by population)...")
    ...
    if not region_noaa_avgs:
        logging.error("Failed to retrieve NOAA data or too many failures. Exiting.")
        return

    # 2) Fetch EIA storage data
    logging.info("Fetching EIA storage data...")
    eia_storage_data = fetch_eia_storage_data()

    # 3) Fetch NG futures data from IB
    logging.info("Fetching NG futures data from IB API...")
    ...
    for region_name, cities in REGIONS.items():
        logging.info(f"Processing forecast for region: {region_name} ...")
        ...
        for city in cities:
            ...
            if forecast_df.empty:
                ...
                fail_count += 1
                if fail_count >= max_failures:
                    logging.critical("Too many forecast failures; aborting.")
                    return
                continue
            city_forecasts[city] = forecast_df

        if not city_forecasts:
            logging.warning(f"No valid city forecasts in {region_name}; skipping region.")
            continue
    ...
    # 6) Print out results (using logging or print as you prefer)
